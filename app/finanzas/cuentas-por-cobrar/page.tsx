...
import { FACTURAS_PENDIENTES, agingBucket, daysFromToday, financeMoney, financeNumber } from "@/lib/finance"
...
export default function CuentasPorCobrarPage() {
  ...
  useEffect(() => {
    ...
    supabase.from("facturas").select("*, proyecto:proyectos(codigo,nombre,deleted_at,cliente:clientes(razon_social))").in("estado", FACTURAS_PENDIENTES).order("fecha_emision", { ascending: true }).then(({ data }) => {
  ...
  const vencidas = facturas.filter(f => daysFromToday(f.fecha_emision) > 30)
  const porVencer = facturas.filter(f => daysFromToday(f.fecha_emision) <= 0)
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>{filtradas.map(f => <tr key={f.id} style={{ borderTop: "1px solid #E2E8F0" }}>
              ...
              <td style={{ padding: 12 }}><StatusBadge label={agingBucket(f.fecha_emision)} type={daysFromToday(f.fecha_emision) > 90 ? "cancelado" : "pendiente"} /></td>
