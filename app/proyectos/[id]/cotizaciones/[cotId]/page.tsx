...
function calcItem(item: any) {
  ...
  return {
    ...item,
    costo_base_calculado: costoBase,
...
export default function CotizacionEditorPage() {
  ...
  function addExtra(itemId: string, grupo: string) {
    setItems(prev => prev.map(item => {
      ...
      return calcItem({ ...item, [grupo]: [...(item[grupo] || []), { label: "", monto: 0, proveedor_id: null, proveedor_nombre: "" }] })
    }))
  }
  ...
  async function guardar(nuevoEstado?: string) {
    ...
    await supabase.from("cotizaciones").update({
      ...
      subtotal_precio_cliente: totalPrecioCliente,
      fee_agencia_monto: feeMonto,
      fee_agencia_pct: feePct,
      fee_activo: feeActivo,
    ...
    <div style={{ maxWidth: 1400, margin: "0 auto" }}>
                        ...
                        <div style={{ padding: "12px 16px 16px 48px" }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                              <span style={{ fontSize: 11, fontWeight: 700, color: "#0F6E56", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                                Costos internos
                              </span>
                              <span style={{ fontSize: 11, color: "#6b7280" }}>
                                Calculado: <strong>{fmt(item.costo_base_calculado || 0)}</strong>
                              </span>
                              {item.costo_manual !== null && item.costo_manual !== "" && (
                            ...
                            <div>
                              ...
                              {(item.extras_produccion || []).map((extra: any, i: number) => (
                                <div key={i} style={{ display: "flex", gap: 6, marginBottom: 4, alignItems: "center" }}>
                                  ...
                                  <input type="number" style={{ ...inp, width: 90, textAlign: "right" }} value={extra.monto || ""}
                                    placeholder="0"
                ...
                <td colSpan={5} style={{ padding: "10px 12px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <label style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer" }}>
                      ...
                      <span style={{ fontSize: 12, fontWeight: 600, color: feeActivo ? "#374151" : "#9ca3af" }}>
                        Fee de agencia
                      </span>
                    </label>
                    {feeActivo && (
      ...
      <div style={{ background: "#E1F5EE", border: "1px solid #1D9E75", borderRadius: 12, padding: "20px 24px", display: "flex", gap: 0, flexWrap: "wrap", alignItems: "stretch", marginTop: 16 }}>
        {[
          ...
          <div key={t.label} style={{ display: "flex", flexDirection: "column", gap: 2, paddingRight: 24, marginRight: 24,
            borderRight: i < arr.length - 1 ? "1px solid #a7f3d0" : "none" }}>
