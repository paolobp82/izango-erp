...
import { buildPresupuestoPdfFilename } from "@/lib/pdf-filename"
      ...
      <Page size="A4" style={s.page}>
        <View style={s.header} fixed>
          ...
          <View style={s.headerRight}>
            <Text style={s.proformaTitle}>PRESUPUESTO</Text>
            <Text style={s.headerSub}>Presupuesto N° {proyecto?.codigo}-V{cotizacion?.version}</Text>
...
export default function PreviewCotizacionPage() {
  ...
  const descargarPDF = async () => {
    ...
    try {
      ...
      a.href = url
      a.download = buildPresupuestoPdfFilename({
        numeroPresupuesto: proyecto?.codigo,
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", background: "#fff", borderRadius: 12, boxShadow: "0 4px 24px rgba(0,0,0,0.08)", overflow: "hidden" }}>
        <div style={{ background: COLOR_PRIMARY, padding: "28px 40px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 22, fontWeight: 800, color: COLOR_DARK, letterSpacing: "-0.5px" }}>PRESUPUESTO</div>
            <div style={{ fontSize: 14, fontWeight: 600, color: COLOR_DARK, marginTop: 2 }}>Presupuesto N° {proyecto?.codigo}-V{cotizacion?.version}</div>
