...
export default function NuevoProyectoPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { data, error } = await supabase.from("proyectos").insert({
      ...
      descripcion_requerimiento: form.descripcion_requerimiento,
      ...
      <form onSubmit={handleSubmit}>
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 14 }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {[{ value: "peru", label: "Izango Peru" }, { value: "selva", label: "Izango Selva" }].map(e => (
                  <button key={e.value} type="button" onClick={() => handleEntidadChange(e.value)}
                    style={{
                      flex: 1, padding: "8px", borderRadius: 8, fontSize: 13,
                      border: form.entidad === e.value ? "2px solid #0F6E56" : "1px solid #e5e7eb",
                      background: form.entidad === e.value ? "#f0fdf4" : "#fff",
                      color: form.entidad === e.value ? "#0F6E56" : "#6b7280",
                      fontWeight: form.entidad === e.value ? 700 : 400,
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <input style={inputStyle} placeholder="Ej: Activacion Honda Mayo Lima" value={form.nombre} onChange={e => f("nombre", e.target.value)} required />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 14 }}>
            <div>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={labelStyle}>Descripcion del requerimiento</label>
            <textarea style={{ ...inputStyle, resize: "vertical" }} rows={3} value={form.descripcion_requerimiento} onChange={e => f("descripcion_requerimiento", e.target.value)} />
          </div>
          <div>
            <label style={labelStyle}>Presupuesto referencial (S/)</label>
            <input type="number" style={inputStyle} placeholder="0.00" value={form.presupuesto_referencial} onChange={e => f("presupuesto_referencial", e.target.value)} />
          </div>
        </div>
        <div className="card" style={{ marginBottom: 16 }}>
