...
export default function ImportExport({ modulo, campos, datos, onImportar, variant = "v1" }: ImportExportProps) {
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h2
                style={
                  isV2
                    ? { fontSize: 17, fontWeight: 900, margin: 0, color: "var(--v2-text)" }
              ...
              <button
                ...
                style={{ background: "none", border: "none", cursor: "pointer", color: "var(--v2-subtle)", fontSize: 22 }}
              >
                ×
              </button>
            </div>
              ...
              <div>
                <div
                  style={
                    isV2
                      ? {
                          background: resultado.errores.length === 0 ? "rgba(22, 163, 74, 0.12)" : "rgba(217, 119, 6, 0.12)",
                          border: "1px solid " + (resultado.errores.length === 0 ? "rgba(22, 163, 74, 0.3)" : "rgba(217, 119, 6, 0.3)"),
                          borderRadius: 10,
                          padding: 16,
                          marginBottom: 16,
                        }
                      : {
                          background: resultado.errores.length === 0 ? "#f0fdf4" : "#fef9c3",
                          border: "1px solid " + (resultado.errores.length === 0 ? "#bbf7d0" : "#fde68a"),
                          borderRadius: 10,
                          padding: 16,
                          marginBottom: 16,
                        }
                ...
                >
                  <div
                    style={{
                      fontSize: 15,
                      fontWeight: 800,
                      color: resultado.errores.length === 0 ? "var(--v2-success)" : "var(--v2-warning)",
                    }}
                  >
                  ...
                  {resultado.errores.length > 0 && (
                    <div style={{ marginTop: 8 }}>
                      <div style={{ fontSize: 12, fontWeight: 800, color: "var(--v2-warning)", marginBottom: 4 }}>
                        {resultado.errores.length} errores:
                      </div>
                      {resultado.errores.map((e: string, i: number) => (
                        <div key={i} style={{ fontSize: 12, color: "var(--v2-muted)" }}>
                          • {e}
                        </div>
                      ))}
                    </div>
              ...
              <div style={{ display: "grid", gap: 16 }}>
                ...
                >
                  <div
                    style={
                      isV2
                        ? { fontSize: 13, fontWeight: 800, color: "var(--v2-text)", marginBottom: 8 }
                ...
                >
                  ...
                  <input
                    ...
                    style={{ fontSize: 13, fontFamily: "inherit", color: isV2 ? "var(--v2-text)" : "inherit" }}
                  />
                  <p style={{ fontSize: 11, color: "var(--v2-muted)", margin: "8px 0 0" }}>
                ...
                {errores.length > 0 && (
                  ...
                  >
                    <div style={{ fontSize: 12, fontWeight: 800, color: "var(--v2-danger)", marginBottom: 6 }}>
                      Errores de validación:
                    </div>
                    {errores.slice(0, 5).map((e, i) => (
                      <div key={i} style={{ fontSize: 12, color: "var(--v2-text)" }}>
                        • {e}
                      </div>
                    ))}
                    {errores.length > 5 && (
                      <div style={{ fontSize: 12, color: "var(--v2-text)" }}>... y {errores.length - 5} más</div>
                    )}
                  </div>
                )}
                  ...
                  <div>
                    ...
                    >
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                        <thead>
                          <tr style={{ background: isV2 ? "var(--v2-surface-soft)" : "#f9fafb" }}>
                          ...
                          {preview.map((row, i) => (
                            <tr key={i} style={{ borderTop: "1px solid " + (isV2 ? "var(--v2-border-soft)" : "#f3f4f6") }}>
                              {campos.slice(0, 4).map((c) => (
                                <td
                                  key={c.key}
                                  style={{ padding: "8px 12px", color: isV2 ? "var(--v2-text)" : "#374151" }}
                                >
                                  {row[c.key] || "—"}
                                </td>
                              ))}
                    ...
                    <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
                      ...
                      <button
                        ...
                        disabled={importing || errores.length > 0}
                        className={isV2 ? "" : "btn-primary"}
                        style={
                          isV2
                            ? {
